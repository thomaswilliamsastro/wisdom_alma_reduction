...
LIST=("
https://almascience.eso.org/dataPortal/2022.1.00141.S_uid___A001_X2d1f_X9e5_001_of_001.tar
https://almascience.eso.org/dataPortal/2022.1.00141.S_uid___A001_X2d1f_X9e5_auxiliary.tar
https://almascience.eso.org/dataPortal/2022.1.00141.S_uid___A002_X1086384_Xc53b.asdm.sdm.tar
https://almascience.eso.org/dataPortal/2022.1.00141.S_uid___A002_X108c378_Xd51.asdm.sdm.tar
https://almascience.eso.org/dataPortal/2022.1.00141.S_uid___A002_X10924ab_X57f3.asdm.sdm.tar
https://almascience.eso.org/dataPortal/member.uid___A001_X2d1f_X9e5.README.txt.tar
")
...
function start_session {
  ...
  if [ "tgwilliams" != "anonymous" ]; then
    ...
    if [ -z "${password}" ]; then
      echo ""
      echo -n "Please enter the password for ALMA account tgwilliams: "
      read -s password
      echo ""
      export password
    fi
    ...
    if command -v "wget" > /dev/null 2>&1; then
      login_command=(wget --quiet --delete-after --no-check-certificate --auth-no-challenge --keep-session-cookies --save-cookies alma-rh-cookie.txt "--http-user=tgwilliams" "--http-password=${password}")
    elif command -v "curl" > /dev/null 2>&1; then
      login_command=(curl -s -k -o /dev/null -c alma-rh-cookie.txt --fail "-u" "tgwilliams:${password}")
    fi
...
echo "Downloading the following files in up to 5 parallel streams. Total size is 4GB."
echo "${LIST}"
